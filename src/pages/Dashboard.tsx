...
const Dashboard: React.FC = () => {
  const { tasks, events, updateTask, isLoading, error } = useApp();
  ...
  const {
    ...
    return {
      todaysTasks: tasksArray.filter(task => isToday(parseISO(task.dueDate))),
      todaysEvents: eventsArray.filter(event => isToday(parseISO(event.date))),
      tasksByStatus: {
      ...
      eventsByType: {
        ...
        reminder: eventsArray.filter(event => event.type === 'reminder').length,
      },
    };
  }, [tasks, events]);
